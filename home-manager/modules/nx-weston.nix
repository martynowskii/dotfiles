...
let
  nx-weston = pkgs.writeShellApplication {
    ...
    text = ''
      # NX родом из 2014 года и про HiDPI не знает: внутри Weston держим
      # масштаб 1, чтобы он получил все физические пиксели, а не растянутую
      # картинку. Оба параметра переопределяются переменными окружения.
      : "''${NX_WESTON_SCALE:=1}"
      : "''${NX_WESTON_ARGS:=--fullscreen}"

      if ! command -v nx >/dev/null 2>&1; then
        echo "nx-weston: команда nx не найдена в PATH." >&2
        echo "Собери и установи пакет из ~/Documents/univer/Siemens/nix" >&2
