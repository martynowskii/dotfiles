...
EOF
    ...
    # Ширина борта рамки. 4 — чтобы окно было за что ухватить мышью.
    #
    # NX_FRAME_BORDER=0 имеет смысл при работе с закреплённой панелью
    # Resource Bar: у NX есть свой разделитель (в libugui.so это
    # dockedControlBarVerticalSplitter со своим курсором splitter_finish),
    # и именно он меняет ширину панели так, чтобы рабочая область
    # подвинулась. Рамка менеджера сидит вплотную снаружи от него и мешает
    # прицелиться. Ценой ухода борта в ноль окно перестанет менять размер
    # перетаскиванием края — но это как раз то перетаскивание, которое до
    # раскладки NX не доходит.
    : "''${NX_FRAME_BORDER:=4}"
