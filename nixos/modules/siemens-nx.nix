...
EOF
    ...
    # Ширина борта рамки. Ноль — потому что борт мешает больше, чем помогает.
    #
    # У NX есть свой разделитель для закреплённой панели Resource Bar (в
    # libugii.so это dockedControlBarVerticalSplitter со своим курсором
    # splitter_finish), и только он меняет ширину панели так, чтобы рабочая
    # область подвинулась следом. Рамка менеджера садится вплотную снаружи
    # от него, и указатель находит сначала её — а перетаскивание рамки до
    # логики стыковки NX не доходит вовсе.
    #
    # Плата: окна перестают менять размер перетаскиванием края. Это ровно то
    # перетаскивание, которое и так ничего полезного не делало; у metacity
    # остаётся изменение размера с клавиатуры (Alt+F8).
    #
    # NX_FRAME_BORDER=4 вернёт борт, за который можно ухватиться.
    : "''${NX_FRAME_BORDER:=0}"
...
decoration, decoration:backdrop {
  margin: 0 ''${NX_FRAME_BORDER}px ''${NX_FRAME_BORDER}px ''${NX_FRAME_BORDER}px;
  box-shadow: none;
  border-radius: 0;
}
...
/* Без -gtk-icon-size: это свойство GTK4, GTK3 его не знает и отбрасывает
   всё правило с предупреждением в консоль при каждом запуске. Размер
   значков в заголовке им и не менялся — те 2 пикселя, что когда-то
   списали на иконки, дал размер шрифта строкой выше. */
